...
battserial = os.popen("ioreg -rd1 -c AppleSmartBattery | awk -F\\\" '/BatterySerialNumber/ { print $(NF-1) }'").read().splitlines()[0]
...
for scope in scopes:
    c.execute(
        'SELECT time,percentage from {}_batteryhistory where uuid="{}"'.format(
            scope, battserial))
    data = c.fetchall()
    rates = []
